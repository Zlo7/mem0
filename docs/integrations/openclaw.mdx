...
Control which sessions are excluded from auto-recall and auto-capture. Manual memory tools remain available even for ignored sessions.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `ignoreCron` | `boolean` | `false` | Skip all cron job sessions |
| `ignoreSessionKeys` | `array<string>` | `[]` | Exact session keys to exclude |
| `ignoreSessionKeyPrefixes` | `array<string>` | `[]` | Session key prefixes to exclude (matched against both full key and rest portion) |
...
<Note>
Session keys follow the format `agent:<agentId>:<rest>`. Prefix matching checks both the full key and the portion after `agent:<agentId>:` for flexible matching.

Trivial message filtering uses `startsWith` matching on user message content (after trimming leading whitespace). Assistant messages are not filtered.
</Note>
